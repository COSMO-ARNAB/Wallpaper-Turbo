...
public class GitHubReleaseProviderOrderingTests
{
    ...
    [Fact]
    public async Task GetLatestManifest_StableCachesUpdateJsonWithIfNoneMatchOnRepeatCall()
    {
        const string releaseJson = """
        {
            "id": 1,
        ...
        var updateJsonRequests = handler.RequestUris
            .Where(u => u.Contains("update.json", StringComparison.OrdinalIgnoreCase))
            .ToList();

        Assert.Equal(2, updateJsonRequests.Count);
        Assert.Equal(2, handler.IfNoneMatchHeaders.Count(h => !string.IsNullOrWhiteSpace(h) && h.Contains("etag-1", StringComparison.OrdinalIgnoreCase)));
    }
